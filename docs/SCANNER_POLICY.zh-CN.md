...
- 模板不会启用全盘扫描、文件内容读取、脚本执行、MCP 启动或扩大扫描范围；Phase 3A/3B 的 SQLite 资源库只保存用户授权扫描产生的安全元数据。
...
- 不把 Skills、MCP、Scripts、Reports、Policies、Validators、Legacy 等静态 snapshot 模块迁移到动态 SQLite 查询。
...
- 将 Skills、MCP、Scripts、Reports、Policies、Validators、Legacy 静态 snapshot 模块迁移到 SQLite；该工作仍属于后续 Phase 3C。
