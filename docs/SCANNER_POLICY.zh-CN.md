...
当前用户-facing 产品定义见 `docs/product/`。本文保留扫描、存储和工程边界术语，用于实现和验收。普通用户界面应使用“查找”“来源”“基本信息”“需要处理”“高级”等简单词，不把脚本、报告、策略、验证器、旧示例或原始诊断作为主功能。
...
普通 `Custom Directories` 来源创建只接受普通自选目录 profiles：`custom-folder`、`project-root`、`ai-toolchain`、`skills-prompts-workspace`、`docs-reports-workspace`、`aios-workspace`。`intelligent-discovery` 与 `advanced-full-disk` profiles 只能由对应发现模式创建和使用，不能通过 custom-directory 请求路径进入。
...
- `depth_limit`：达到 profile max depth 后不继续遍历后代条目。
- `cancelled`：用户取消后在遍历检查点停止。
- `unsupported_root`：发现候选或请求根不支持，未作为扫描来源。
- `duplicate_source`：重复来源被复用或跳过，未创建新的扫描范围。
...
Phase 3C 将 Phase 3A/3B 写入的 SQLite 本地资源库作为主要产品数据源。Dashboard、Skills、MCP、Scripts、Reports、Project Packs、Policies、Validators 和资源检查器读取动态资源语料；当本地库为空时，默认产品模块显示空资源库和 0 计数。内置 JSON snapshot 只作为 Legacy 示例/兼容数据显式查看，不代表当前电脑扫描结果。
...
- 动态资源被移除、来源被删除或本地库被清空后，默认模块回到空动态视图，不保留断裂选择；示例 snapshot 仅在 Legacy 入口查看。
