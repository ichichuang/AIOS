...
本步骤只新增文档，不新增 GitHub workflow，不创建 tag 或 release，不推送、不写入 secrets，不改变产品行为。
...
## 后续 workflow_dispatch artifact 流程

当 GitHub Actions packaging workflow 被单独批准并添加后，内部发行顺序应为：
...
- 增加明确 Windows bundle script。
- 增加 Windows runner artifact workflow。
...
- `.github/workflows/aios-desktop-packaging.yml`
- `package.json`
