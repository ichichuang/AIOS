...
Phase 3A 已实现本地资源库基础：Rust 后端拥有 SQLite 数据库、v1 schema migration、终态 scan job persistence、只读查询 commands、资源库摘要 UI 和清空本地库控制。Phase 3B 已在该基础上增加多目录扫描来源管理、项目标签、启用状态、来源删除、顺序批次扫描、批次取消和来源级持久摘要。静态 snapshot 模块动态迁移仍未开始。
...
- idempotent v1/v2 schema migrations。
- typed Tauri commands：资源库状态、扫描来源、扫描任务、资源摘要、资源列表、来源更新/删除、批次扫描/取消/快照和清空本地库。
...
- 全盘扫描、后台静默扫描、diff view 或静态模块动态迁移。
...
- full-disk discovery 仍不属于 Phase 3C 默认目标；如进入高级模式，必须另起权限门控设计。
