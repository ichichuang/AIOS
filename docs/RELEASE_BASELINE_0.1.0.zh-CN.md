...
Phase 4C.3 修正说明：Dashboard 作为“本机资源库”入口展示动态 SQLite 是否为空、动态资源总数、扫描来源数、项目 scope 数、最近扫描状态/时间和当前查看 scope。新增产品层面的 Project Resource Map 与 Scan Source Directory Map，分别展示项目标签、关联目录、资源类型计数、最近扫描状态、跳过/错误计数，并只能切换到对应 scope，不启动扫描。Global / Project / Source / Unclassified / Legacy 的语义在顶部 scope 选择器和模块标题附近明确展示；Legacy 仍仅为内置示例/兼容快照，不代表当前电脑扫描结果。

Phase 4C.4 产品 UX 验收说明：空首次使用、动态 two-project / two-source scope、Legacy 隔离和扫描安全验收记录见 `docs/PRODUCT_UX_ACCEPTANCE_0.1.0.zh-CN.md`。本阶段只补充验收、文档和小范围分类修正，不扩大扫描范围、不新增 Tauri 插件、不执行真实 root/home/system/full-disk 扫描。

Phase 4C.6 原生 app-data 验收说明：隔离 app data 启动、空首次使用、Legacy 隔离、重启持久化和 Rust native fixture scan/store/reset 路径记录见 `docs/NATIVE_APP_DATA_ACCEPTANCE_0.1.0.zh-CN.md`。该验收为有条件通过；完整 native picker fixture scan 仍受当前 macOS Accessibility / foreground 自动化限制，需在可稳定控制的会话中补做人工或自动化 smoke。
...
- Phase 4C.3：本机资源库、项目视角、来源目录视角与 Legacy 语义产品化修正。
- Phase 4C.4：首次使用、本机资源库、项目/来源 scope、Legacy 隔离和扫描安全产品 UX 验收。
- Phase 4C.6：隔离 app data 原生验收（有条件通过；native picker fixture scan 需补做稳定 UI smoke）。
...
- Product resource maps：Dashboard 展示 Local Resource Library、Project Resource Map 和 Scan Source Directory Map；卡片只切换 scope 或导航到扫描管理，不启动扫描。
...
- 当前 Phase 4C.6 已通过隔离 app data 和 Rust native fixture path 验收，但未完成完整 native picker fixture scan。
