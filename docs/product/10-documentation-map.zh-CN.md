...
| 文档 | 权威内容 |
| --- | --- |
| `docs/product/00-aios-desktop-product-definition.zh-CN.md` | **最高权威产品定义**：产品身份、目标用户、六大模块、术语规则、安全承诺、验收原则。 |
...
1. 先遵守仓库治理与安全规则。
2. 再遵守 `docs/product/00-aios-desktop-product-definition.zh-CN.md`。
3. 再参考 `docs/product/` 其余文件和 README。
4. 再参考支持文档。
5. 内部工程、发行、验收和历史文档只作为实现或追溯证据。
6. 如果旧文档出现旧模块名，应理解为历史实现或高级内部信息，不应进入普通用户主导航。
