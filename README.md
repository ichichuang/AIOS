...
AIOS Desktop 的产品化方向记录在以下文档中。当前仓库已进入 Phase 4C 本地 unsigned 产品验收范围，继续复用现有 Material Console 前端并保留最小 Tauri v2 权限边界。
...
- `docs/PRODUCT_UX_ACCEPTANCE_0.1.0.zh-CN.md`

当前桌面壳承载现有 Material Console 前端，并已进入 Phase 4C 本地 unsigned 产品验收范围，包含多目录扫描管理、静态扫描模板、智能全机发现、高级全盘发现显式确认门控、首次/空库引导、隐私与本地数据控制、当前运行时内的批次进度与取消能力、Rust-owned SQLite 本地资源库，以及动态资源语料优先展示。未扫描任何目录时，Skills/MCP/Scripts/Reports/Policies/Validators 等默认模块保持空资源库和 0 计数；内置 snapshot 只在 Legacy 示例/兼容入口查看，不代表当前电脑扫描结果。Tauri SQL、文件系统、Shell 插件、MCP 执行、启动自动扫描和未确认全盘扫描仍未启用。
