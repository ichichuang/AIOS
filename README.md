...
当前桌面壳承载现有 Material Console 前端，并包含多目录扫描管理、静态扫描模板、智能全机发现、高级全盘发现显式确认门控、当前运行时内的批次进度与取消能力、Rust-owned SQLite 本地资源库，以及动态资源语料优先展示。未扫描任何目录时，Skills/MCP/Scripts/Reports/Policies/Validators/Legacy 等模块继续回退到内置 snapshot；Tauri SQL、文件系统、Shell 插件、MCP 执行、启动自动扫描和未确认全盘扫描仍未启用。
